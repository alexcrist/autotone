...
export const getBufferSize = (sampleRate) => {
  const minBufferSize = sampleRate / MODEL_SAMPLE_RATE * MODEL_MIN_WINDOW_SIZE;
...
export const detectPitch = (buffer) => {
  let freq;
  let confidence;
...
const resample = (buffer) => {
  const interpolate = _sampleRate % MODEL_SAMPLE_RATE !== 0;
  const multiplier = _sampleRate / MODEL_SAMPLE_RATE;
